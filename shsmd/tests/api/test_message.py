...
class MessageTestCase(common.ShsmdTestCase):
    ...
    def test_send_message_unsigned_destination_usernames(self):
        ...
        self.add_key(username, device_public_key.decode('utf-8'), device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'),
                          b64encode(device_signing_key.sign(device_public_key)).decode('utf-8'))
    ...
    def test_send_message_badsigned_destination_usernames(self):
        ...
        self.register_device(username, device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), valid_device_verify_key)
        self.add_key(username, public_key.decode('utf-8'), device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'),
                          b64encode(device_signing_key.sign(public_key)).decode('utf-8'))
        rv = self.send_message(device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), b64encode(master_signing_key.sign('a'.encode())).decode('utf-8'), b64encode(device_signing_key.sign(public_key)).decode('utf-8'), b64encode(device_signing_key.sign('a'.encode())).decode('utf-8'))
        assert isinstance(rv, flask.wrappers.Response)
        response = json.loads(rv.data.decode('utf-8'))
        assert rv.status_code == 400
        assert 'message' in response.keys()
    ...
    def test_send_message_badsigned_message_public_key(self):
        ...
        self.add_key(username, public_key.decode('utf-8'), device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'),
                          b64encode(device_signing_key.sign(public_key)).decode('utf-8'))
        rv = self.send_message(device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), b64encode(device_signing_key.sign('a'.encode())).decode('utf-8'), b64encode(master_signing_key.sign(public_key)).decode('utf-8'), b64encode(device_signing_key.sign('a'.encode())).decode('utf-8'))
        assert isinstance(rv, flask.wrappers.Response)
        response = json.loads(rv.data.decode('utf-8'))
        assert rv.status_code == 400
        assert 'message' in response.keys()
    ...
    def test_send_message_badsigned_message_contents(self):
        ...
        self.add_key(username, public_key.decode('utf-8'), device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'),
                          b64encode(device_signing_key.sign(public_key)).decode('utf-8'))
        rv = self.send_message(device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), b64encode(device_signing_key.sign('a'.encode())).decode('utf-8'), b64encode(device_signing_key.sign(public_key)).decode('utf-8'), b64encode(master_signing_key.sign('a'.encode())).decode('utf-8'))
        assert isinstance(rv, flask.wrappers.Response)
        response = json.loads(rv.data.decode('utf-8'))
        assert rv.status_code == 400
        assert 'message' in response.keys()
    ...
    def test_send_message_invalid_json_destination_usernames(self):
        ...
        self.add_key(username, public_key.decode('utf-8'), device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'),
                          b64encode(device_signing_key.sign(public_key)).decode('utf-8'))
        rv = self.send_message(device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), b64encode(device_signing_key.sign('a'.encode())).decode('utf-8'), b64encode(device_signing_key.sign(public_key)).decode('utf-8'), b64encode(device_signing_key.sign('a'.encode())).decode('utf-8'))
        assert isinstance(rv, flask.wrappers.Response)
        response = json.loads(rv.data.decode('utf-8'))
        assert rv.status_code == 400
        assert 'message' in response.keys()
    ...
    def test_send_message_invalid_list_destination_usernames(self):
        ...
        self.add_key(username, public_key.decode('utf-8'), device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'),
                          b64encode(device_signing_key.sign(public_key)).decode('utf-8'))
        rv = self.send_message(device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), b64encode(device_signing_key.sign(json.dumps('a').encode())).decode('utf-8'), b64encode(device_signing_key.sign(public_key)).decode('utf-8'), b64encode(device_signing_key.sign('a'.encode())).decode('utf-8'))
        assert isinstance(rv, flask.wrappers.Response)
        response = json.loads(rv.data.decode('utf-8'))
        assert rv.status_code == 400
        assert 'message' in response.keys()
    ...
    def test_send_message_valid(self):
        ...
        self.add_key(username, public_key.decode('utf-8'), device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'),
                          b64encode(device_signing_key.sign(public_key)).decode('utf-8'))
        rv = self.send_message(device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), b64encode(device_signing_key.sign(json.dumps([username]).encode())).decode('utf-8'), b64encode(device_signing_key.sign(public_key)).decode('utf-8'), b64encode(device_signing_key.sign('a'.encode())).decode('utf-8'))
        assert isinstance(rv, flask.wrappers.Response)
        response = json.loads(rv.data.decode('utf-8'))

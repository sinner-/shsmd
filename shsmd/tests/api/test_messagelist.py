...
class MessageTestCase(common.ShsmdTestCase):
    def test_get_messages_empty_signed_device_verify_key(self):
        rv = self.get_messages(None, None, None)
    ...
    def test_get_messages_nonexistent_signed_device_verify_key(self):
        ...
        valid_device_public_key = b64encode(master_signing_key.sign(public_key)).decode('utf-8')
        ...
        rv = self.get_messages(username, master_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), b64encode(device_signing_key.sign('a'.encode())).decode('utf-8'))
        assert isinstance(rv, flask.wrappers.Response)
        response = json.loads(rv.data.decode('utf-8'))
    ...
    def test_get_messages_badsigned_signed_device_verify_key(self):
        ...
        self.register_device(username, device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), valid_device_verify_key)
        self.add_key(username, public_key.decode('utf-8'), device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), valid_device_public_key)
        rv = self.get_messages(username, device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), valid_device_verify_key)
        assert isinstance(rv, flask.wrappers.Response)
        response = json.loads(rv.data.decode('utf-8'))
        assert rv.status_code == 400
        assert 'message' in response.keys()
    ...
    def test_get_messages_valid_no_messages(self):
        ...
        self.register_device(username, device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), valid_device_verify_key)
        self.add_key(username, public_key.decode('utf-8'), device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), valid_device_public_key)
        rv = self.get_messages(username, device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), b64encode(device_signing_key.sign(device_signing_key.verify_key.encode(encoder=HexEncoder))).decode('utf-8'))
        assert isinstance(rv, flask.wrappers.Response)
        response = json.loads(rv.data.decode('utf-8'))
    ...
    def test_get_messages_valid(self):
        ...
        self.register_device(username, device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), valid_device_verify_key)
        self.add_key(username, public_key.decode('utf-8'), device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), valid_device_public_key)
        self.send_message(device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), b64encode(device_signing_key.sign(json.dumps([username]).encode())).decode('utf-8'), b64encode(device_signing_key.sign(public_key)).decode('utf-8'), b64encode(device_signing_key.sign('a'.encode())).decode('utf-8'))
        rv = self.get_messages(username, device_signing_key.verify_key.encode(encoder=HexEncoder).decode('utf-8'), b64encode(device_signing_key.sign(device_signing_key.verify_key.encode(encoder=HexEncoder))).decode('utf-8'))
        assert isinstance(rv, flask.wrappers.Response)
        response = json.loads(rv.data.decode('utf-8'))
        assert rv.status_code == 200
